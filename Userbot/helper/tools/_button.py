...
"""
    ...
    def START(message):
        msg = f"""
<blockquote><b>👋 Welcome <a href=tg://user?id={message.from_user.id}>{message.from_user.first_name} {message.from_user.last_name or ''}</a></b>

<b><u>[{nama_bot}](https://t.me/{bot.me.username})</u> adalah Userbot multi-client dan multi-bahasa yang bisa digunakan untuk umum secara gratis tanpa persyaratan apapun.</b></blockquote>
<b>Created by {Ads()}</b>
"""
        return msg
...
❍ Mengikuti prosedur pembuatan dibot ini
❍ Resiko userbot bisa [BACA DISINI](http://telegra.ph/RESIKO-USERBOT-03-07)
                ...
                return await client.answer_inline_query(
                    ...
                    results=[
                        InlineQueryResultArticle(
                            title=f"Anda Belum Mendapatkan Akses Userbot {bot_username}",
                            input_message_content=InputTextMessageContent(
                                f"Kamu Bisa Meminta Akses {bot_username} Agar Bisa Menggunakan"
                            ),
                        )
                    ],
                )
        ...
        async def wrapper(client, cq, *args):
            ...
            if cq.from_user.id not in users:
                return await cq.answer(
                    f"Silakan Minta Akses {bot_username} Agar Bisa Menggunakan Bot Ini",
                    True,
                )
            else:
